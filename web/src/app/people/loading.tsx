function SkeletonCard() {
  return (
    <div className="border border-border p-5 animate-pulse">
      ...
      <div className="mt-3 h-5 w-16 rounded bg-border" />
    </div>
  );
}
...
export default function PeopleLoading() {
  return (
    <div className="mx-auto max-w-6xl px-6 py-12">
      <div className="h-8 w-32 rounded bg-border animate-pulse" />
      <div className="mt-2 h-5 w-64 rounded bg-border animate-pulse" />

      <div className="mt-6 flex gap-1">
        {Array.from({ length: 27 }).map((_, i) => (
          <div key={i} className="h-7 w-7 rounded bg-border animate-pulse" />
        ))}
      </div>
